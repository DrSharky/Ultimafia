...
const port: string = process.env.CHAT_PORT!;
const server = new sockets.SocketServer(port);
const users: { [key: string]: any } = {}; // TODO: Provide a proper type instead of 'any'.
const channelMembers: { [key: string]: any } = {}; // TODO: Provide a proper type.
...
(async function () {
  try {
    ...
    process
      .on("unhandledRejection", (err) => logs.logger.error(err))
      .on("uncaughtException", (err) => logs.logger.error(err))
      .on("exit", onClose)
      .on("SIGINT", onClose)
      .on("SIGUSR1", onClose)
      .on("SIGUSR2", onClose);
